...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     */
     ...
     fillhisto[1]=(std::find(event_hlt_bit->begin(), event_hlt_bit->end(), 194)!=event_hlt_bit->end());
     fillhisto[2]=(std::find(event_hlt_bit->begin(), event_hlt_bit->end(), 195)!=event_hlt_bit->end());
     fillhisto[3]=(std::find(event_hlt_bit->begin(), event_hlt_bit->end(), 205)!=event_hlt_bit->end());
     fillhisto[4]=fillhisto[1]||fillhisto[2]||fillhisto[3];
     fillhisto[5]=false;
     fillhisto[6]=false;
